...
const PatientOnboardingModal: React.FC<PatientOnboardingModalProps> = ({ 
  isOpen, 
  onClose, 
  prescriptionData,
  onComplete 
    ...
    setTimeout(async () => {
      if (formData.otp === '123456') { // Mock OTP
        ...
        const userData = {
          ...
          dateOfBirth: '',
          gender: prescriptionData?.patientInfo?.gender || 'Male'
        };
        
        // Login user
        login('patient', `patient_${Date.now()}`, userData);
        
        // Add prescription to health records
        if (prescriptionData) {
          await addPrescriptionRecord(prescriptionData, formData.mobile);
        }
  ...
  const handleSkipRegistration = () => {
    ...
    login('patient', 'guest_user', guestData);
    
    if (prescriptionData) {
      addPrescriptionRecord(prescriptionData, formData.mobile);
    }
